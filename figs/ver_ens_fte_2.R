...
for (i in seq(1,11,5)){
  ...
  j <- round(i/5)+1
  df <- rank_tab %>% filter(s1==a1, s2==a2[j], tau==tau_)
  ...
  if (j == 1) {
    p <- p + annotate("text", x=0.48, y=1.15, size=3.5, label=params$params) 
  } else if (j == 2) {
    p <- p + ylim(0, 1.25) +
      annotate("text", x=0.48, y=1.2, size=3.5, label=params$params)
  } else {
    p <- p + ylim(0, 1.3) +
      annotate("text", x=0.48, y=1.25, size=3.5, label=params$params)
  }
...
png('ver_ens_fte_2.png', units='in', width=8, height=5, res=400, pointsize=9)
grid.draw(grd)
dev.off()
