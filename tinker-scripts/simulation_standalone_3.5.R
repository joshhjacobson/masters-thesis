...
range_sim <- function(s_1, N, seed, fname) {
  ...
  rank_stats <- try(read.table(paste("rank_stats_s", s_1, ".RData", sep="")))
  if("try-error" %in% class(rank_stats)) rank_stats <- initDF(s_2=s_2, tau=tau)
  ...
  for (ii in s_2) {
    ...
    ## write out results after each s2 col completed
...
rank_stats <- range_sim(s_1[idx], N, seed[idx], nam[idx])

## Remote
# save(rank_stats,
#      file = paste(nam[idx], ".RData", sep = ""))
